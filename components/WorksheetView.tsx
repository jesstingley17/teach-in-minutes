...
export const WorksheetView: React.FC<WorksheetViewProps> = ({ 
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`${worksheet.title.replace(/\s+/g, '_')}_Print.pdf`);
    ...
    <div className="relative antialiased pb-4 mt-8 bg-slate-50 min-h-screen">
      <div className="absolute -top-12 left-0 right-0 flex justify-between items-center no-print px-4 py-2 bg-white border border-slate-100 rounded-xl shadow-sm z-[60]">
        <div className="flex gap-4 items-center">
          ...
          <button onClick={handleExportPDF} disabled={isExporting} className="flex items-center gap-2 px-4 py-1.5 rounded-lg font-black text-[9px] uppercase tracking-widest bg-slate-900 text-white shadow-sm disabled:opacity-50 hover:bg-slate-800">
      ...
      <div id="worksheet-content" className="max-w-[210mm] mx-auto min-h-[297mm] bg-white text-slate-900 overflow-hidden relative border border-slate-200 shadow-2xl">
        ...
        <div className="p-16 border-b-2 border-slate-900 bg-white">
           ...
           <div className="flex justify-between items-end gap-12">
              ...
              <div className="text-right">
                 <p className="text-[7px] font-black uppercase tracking-widest opacity-40 mb-1">Total Pts</p>
        ...
        <div className="px-16 py-12 space-y-12 bg-white">
                ...
                <div className="col-span-12">
                   <SketchyBorderBox className="bg-white border border-slate-200 transform rotate-[-0.3deg]">
                      ...
                      <div className="text-sm font-medium leading-relaxed">
                         <EditableField multiline value={worksheet.teachingContent || "Synthesizing source knowledge for student mastery..."} onSave={(v: any) => handleUpdate({...worksheet, teachingContent: v})} />
                      </div>
                   </SketchyBorderBox>
                </div>
                <div className="col-span-12 md:col-span-8">
                  <div className="bg-slate-50/50 p-8 rounded-3xl border-2 border-dashed border-slate-200">
                     <h3 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest mb-4"><Star className="w-4 h-4 text-yellow-500 fill-yellow-500" /> Mastery Takeaways</h3>
                     <ul className="space-y-3">
                        {(worksheet.keyTakeaways || ["Takeaway 1", "Takeaway 2", "Takeaway 3", "Takeaway 4"]).map((takeaway, i) => (
           ...
           <div className="mt-32 pt-8 border-t-2 border-slate-100 flex justify-between items-end opacity-20 text-[6px] font-black uppercase tracking-[0.4em]">
              <div>{worksheet.institutionName} • Protocol {worksheet.id?.slice(-4).toUpperCase()}</div>
              <div className="text-right">{worksheet.learnerProfile} Scaffolding Active</div>
           </div>
        </div>
