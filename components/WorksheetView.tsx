...
import { Worksheet, QuestionType, ThemeType, Question, AudienceCategory, LayoutStyle } from '../types';
import { LatexRenderer } from './LatexRenderer';
import { SketchyBorderBox } from './HandwritingElements';
import html2canvas from 'html2canvas';
import { jsPDF } from 'jspdf';
import { 
  Trash2, Edit3, Check, Landmark, Printer, Loader2, ShieldCheck, GraduationCap, Grid, Info, Scale, PenTool, QrCode, CloudUpload, Lightbulb, BookOpen, Star, FileText
} from 'lucide-react';
...
export const WorksheetView: React.FC<WorksheetViewProps> = ({ 
  ...
  const EditableField = ({ value, onSave, className, style, multiline = false, placeholder = "", isMath = true }: any) => {
    ...
    return (
      <span onClick={() => setEditing(true)} className={`cursor-text p-1 rounded border-2 border-transparent hover:border-blue-100 hover:bg-blue-50 block ${className} ${!value ? 'text-slate-300 italic' : ''}`} style={style}>
  ...
  return (
    <div className="relative antialiased pb-4 mt-8">
      ...
      <div className="absolute -top-12 left-0 right-0 flex justify-between items-center no-print px-4 py-2 bg-white border border-slate-100 rounded-xl shadow-sm z-[60]">
        ...
        <div className="text-[8px] font-black uppercase text-slate-300">Style: {layoutStyle} • Profile: {worksheet.learnerProfile}</div>
      </div>
      ...
      <div id="worksheet-content" className="max-w-[210mm] mx-auto min-h-[297mm] bg-white text-slate-900 overflow-hidden relative border border-slate-200 shadow-2xl">
        ...
        {/* Header Section */}
        <div className={`p-16 border-b-2 border-slate-900 ${layoutStyle === LayoutStyle.ARCHITECT ? 'bg-white' : 'bg-slate-50/30'}`}>
           <div className="flex justify-between items-start mb-12">
              <div className="space-y-4">
                 <div className="h-16 w-auto mb-6 relative group">
              ...
              <div className="text-right">
                 <div className="inline-flex flex-col items-end opacity-20 mb-4"><QrCode className="w-10 h-10" /></div>
                 <div className="text-[8px] font-black uppercase tracking-widest opacity-40">Auth: {worksheet.id?.slice(0, 6).toUpperCase()}</div>
              </div>
           </div>
           ...
           <div className="flex justify-between items-end gap-12">
              <div className="flex-1">
                 ...
                 <h1 className={`font-black uppercase tracking-tighter leading-none ${layoutStyle === LayoutStyle.ARCHITECT ? 'text-4xl' : 'text-5xl'}`}>
              ...
              <div className="text-right">
                 ...
                 <p className="font-black text-3xl" style={{ color: primaryColor }}>{totalPoints}</p>
              </div>
           </div>
        </div>

        <div className="px-16 py-12 space-y-12">
           {/* Conditional "Laid-Teach" Layer */}
           {layoutStyle === LayoutStyle.LAID_TEACH && (
             <div className="grid grid-cols-12 gap-8 animate-in slide-in-from-top-4 duration-500">
                <div className="col-span-12">
                   <SketchyBorderBox className="bg-white shadow-sm border border-slate-100 transform rotate-[-0.5deg]">
                      <div className="flex items-center gap-3 mb-6">
                         <BookOpen className="w-6 h-6" style={{ color: primaryColor }} />
                         <h2 className="text-xl font-black uppercase tracking-tight">Mini-Lesson Summary</h2>
                      </div>
                      <div className="text-sm font-medium leading-relaxed italic">
                         <EditableField multiline value={worksheet.teachingContent || "Instructional summary..."} onSave={(v: any) => handleUpdate({...worksheet, teachingContent: v})} />
                      </div>
                   </SketchyBorderBox>
                </div>
                <div className="col-span-12 md:col-span-7">
                  <div className="bg-yellow-50/50 p-8 rounded-3xl border-2 border-dashed border-yellow-200">
                     <h3 className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-yellow-700 mb-4"><Star className="w-4 h-4" /> Takeaways</h3>
                     <ul className="space-y-3">
                        {(worksheet.keyTakeaways || ["Concept A", "Concept B", "Concept C"]).map((takeaway, i) => (
           ...
           {/* ASSESSMENT PORTION */}
           <div className={`${layoutStyle === LayoutStyle.ARCHITECT ? 'pt-4' : 'pt-12 border-t border-slate-100'} space-y-16`}>
              <h2 className="text-2xl font-black uppercase tracking-tight flex items-center gap-4">
                 <PenTool className="w-6 h-6" /> {layoutStyle === LayoutStyle.ARCHITECT ? 'Assessment Items' : 'Applied Assessment'}
              </h2>
              <div className="space-y-16">
                {worksheet.questions.map((q, idx) => (
                  <div key={q.id}>
                     ...
                     <div className="ml-16">
                        {q.options && (
                           <div className={`grid gap-4 ${layoutStyle === LayoutStyle.ARCHITECT ? 'grid-cols-1' : 'grid-cols-2'}`}>
                              {q.options.map((opt, i) => (
           ...
           <div className="mt-32 pt-8 border-t-2 border-slate-100 flex justify-between items-end opacity-20 text-[6px] font-black uppercase tracking-[0.4em]">
              <div>{worksheet.institutionName} • Protocol {worksheet.id?.slice(-4)}</div>
              <div className="text-right">Aesthetic: {layoutStyle}</div>
           </div>
        </div>
                  ...
                  <div key={`key-${q.id}`} className="flex gap-10 items-start border-l-2 border-slate-200 pl-10">
                     ...
                     <div className="flex-1 space-y-6">
                        <div className="p-6 bg-slate-50 border border-slate-100 rounded-xl">
                           <p className="text-[7px] font-black uppercase text-slate-400 mb-2">Verified Answer</p>
                           <p className="text-xl font-black"><LatexRenderer content={q.correctAnswer} /></p>
                        </div>
                        {q.explanation && (
                           <div className="p-6">
                             <p className="text-[7px] font-black uppercase text-slate-400 mb-2">Rationale</p>
