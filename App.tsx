...
const App: React.FC = () => {
   ...
   * This is requested to show "always connected" status on the main page.
   */
  const checkAIConnectivity = async () => {
    setIsVerifyingKey(true);
    
    // Check for hardcoded/injected key first
    const hasEnvKey = !!process.env.API_KEY && process.env.API_KEY.length > 5;
    
    // Check for platform-selected key
    let hasSelectedKey = false;
    try {
      // @ts-ignore - Platform specific check for Gemini 3 Pro access
      if (window.aistudio?.hasSelectedApiKey) {
        // @ts-ignore
        hasSelectedKey = await window.aistudio.hasSelectedApiKey();
      }
    } catch (e) {
      console.warn("Connectivity check error:", e);
    }
  ...
  useEffect(() => {
    ...
    // Heartbeat every 10 seconds for real-time status as requested
    const interval = setInterval(checkAIConnectivity, 10000);
    return () => clearInterval(interval);
  }, []);

  const handleConnectApiKey = async () => {
    // Per instructions: "add a button which calls await window.aistudio.openSelectKey() to open a dialog"
    // No input field should be generated by the app.
    // @ts-ignore
    if (window.aistudio?.openSelectKey) {
      try {
        // @ts-ignore
        await window.aistudio.openSelectKey();
        // Proceed immediately as the key is injected by the platform
        setApiKeyMissing(false);
      } catch (e) {
        console.error("Key Selection Error", e);
      }
    } else {
      // Fallback message if running outside of AI Studio environment
      alert("Please ensure your API Key is configured in the environment settings of this platform. The key selection dialog is only available in specific AI Studio contexts.");
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (e.message?.includes("Requested entity was not found")) {
        ...
        alert("Suite synthesis encountered a reasoning conflict. Check your prompt constraints.");
      }
           ...
           >
              ...
              <div className="flex-1">
                 ...
                 <p className="text-[7px] font-bold text-slate-400 uppercase tracking-tighter">
                   {apiKeyMissing ? 'Action Required' : 'Gemini 3 Pro reasoning active'}
                 </p>
              </div>
